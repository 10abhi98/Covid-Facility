...
let locationDetails = {
	name: ['Ganga Ram Hospital', 'Medicare Pharmacy', 'Shree Hospital', 'Dharamvir Pharmacy', 'Sant Soham Hospital',],
	address: ['Jawahar Lal Nehru Marg,Delhi.110002', 'Model Town', 'Saket', 'Rohini', 'Sarita Vihar',],
...
class Dashboard extends Component {
	...
	submitInfoHandler(e) {
		...
		console.log(
			...
			'New Patients     ->' + this.state.newPatients + '\n' + 
			'Waiting Patients ->' + this.state.waitingPatients + '\n' +
			'Remidisivir      ->' + this.state.remidisivir + '\n' 
		);
	}
	...
	tasksAssignment = (Locations) => {
		return Locations.map((location, index) => {
			return (
				<button
					key = {location+index}
					className={this.state.activeBtn === location ? 'locationActiveBtn' : 'locationBtn'}
					type='button'
		...
		return Object.entries(type).map((value, index) => {
			...
			return (
				<div className='form-group' key={value[0] + index}>
					...
					<input
						type='number'
						name={val.length === 2 ? val[0].toLowerCase()+val[1] : val[0].toLowerCase()}
						...
						<div id='locationForm' className='col-md-7'>
							<div className = 'p-1'>
								...
								<div id='hospDetails'>
									...
									<p>
										<em>Responding {this.state.response}.</em>
									</p>
									<p>
								...
								<form id='taskList'>
									...
									{this.state.locationName.toLowerCase().includes('hospital')
										? this.displayQuestionarre(hospitalQuestionarre)
										: this.displayQuestionarre(pharmacyQuestionarre)
									}
									{/* Submit Button */}
									<button
										id='submitBtn'

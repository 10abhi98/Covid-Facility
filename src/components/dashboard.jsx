...
class Dashboard extends Component {
    ...
    componentDidMount() {
        ...
        userDocument
            ...
            .then((doc) => {
                ...
                Promise.all([this.fetchTaskAssignTime()]).then((res) => {
    ...
    // Fetch Location Data ->
    fetchLocationData() {
        console.log('called fetchlocations');
    ...
    // Fetch Reassign Time ->
    async fetchTaskAssignTime() {
        this.snackbar('Fetching Tasks', 3000);
        const { currentUser } = this.context;
        if (this.state.tasks.length) {
            await firestore
                ...
                .then((res) => {
                    ...
                    const diff = Date.now() - assignTime * 1000;
                    if (diff >= 0) {
                        firestore
                            .collection('volunteers')
                            .doc(currentUser.uid)
                            ...
                            .then((res) => {
                                ...
                                this.snackbar(
                                    'Tasks Expired..Fetching New Tasks',
                                    6000
                                );
                                this.fetchTasks();
                                this.fetchLocationData();
    ...
    taskTimer() {
        ...
        if (duration.minutes() <= 0) {
            ...
            // this.snackbar('Tasks Expired....Fetching New Tasks');
            this.fetchTasks();
            this.fetchLocationData();
            setTimeout(function () {
                window.location.reload();
    ...
    async submitInfoHandler(e, taskId) {
        ...
        if (this.state.tasks.length === 0) {
            ...
            this.fetchLocationData();
            setTimeout(function () {
                self.snackbar('All Tasks Completed..Fetching New Tasks', 10000);
                window.location.reload();
            }, 500);
        }
        ...
        this.snackbar('Task Completed ..', 2000);
    }
